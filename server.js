...
officeDataRouter.get("/roster", (req, res) => {
  const rows = db.prepare("SELECT id, full_name FROM users WHERE office_id = ? AND role = 'employee' AND active = 1 ORDER BY full_name")
    .all(req.user.office_id);
...
function getAttendanceDay(officeId, date) {
  ...
  const roster = db.prepare("SELECT id, full_name FROM users WHERE office_id = ? AND role = 'employee' AND active = 1 ORDER BY full_name").all(officeId);
