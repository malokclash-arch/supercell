...
superAdminRouter.post("/users", (req, res) => {
  ...
  try {
    const info = db.prepare(`
      ...
      VALUES ($username, $hash, $full_name, $role, $office_id, 1, $created_at)
    `).run({
      $username: username.trim(), $hash: hashPassword(password), $full_name: full_name.trim(),
      $role: role, $office_id: (role === "office_admin" || role === "employee") ? office_id : null, $created_at: new Date().toISOString()
    });
...
officeAdminRouter.get("/employees", (req, res) => {
  const rows = db.prepare("SELECT id, username, full_name, active FROM users WHERE office_id = ? AND role = 'employee' ORDER BY created_at DESC")
...
officeAdminRouter.post("/employees", (req, res) => {
  ...
  try {
    const info = db.prepare(`
      INSERT INTO users (username, password_hash, full_name, role, office_id, active, created_at)
      VALUES ($username, $hash, $full_name, 'employee', $office_id, 1, $created_at)
    `).run({
      $username: username.trim(), $hash: hashPassword(password), $full_name: full_name.trim(),
...
officeDataRouter.get("/roster", (req, res) => {
  const rows = db.prepare("SELECT id, full_name FROM users WHERE office_id = ? AND role IN ('employee','office_admin') AND active = 1 ORDER BY (role='office_admin') DESC, full_name")
    .all(req.user.office_id);
...
function getAttendanceDay(officeId, date) {
  ...
  const roster = db.prepare("SELECT id, full_name FROM users WHERE office_id = ? AND role IN ('employee','office_admin') AND active = 1 ORDER BY (role='office_admin') DESC, full_name").all(officeId);
